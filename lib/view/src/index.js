...
module.exports = function player (state, onclick) {
  ...
  function dial () {
    if (state.player.looping) {
      ...
      var dialFatness = .03
      var dialRgba = `rgba(0,0,0, ${state.player.gain})`
      var s = progress-dialFatness
      var e = progress+dialFatness
      return arc(center, radius, thickness, s, e, dialRgba)
    }
    return
  }
  ...
  function error () {
    let err = state.player.error
    if (err)
      return yo`<div style="color:red;">${err.toString()}</div>`
    return
  }
  ...
  ${ring(center, radius, thickness, state.style.color, 'transparent')}
  ${progress()}
  ${dial()}
    </svg>
  ${error()}
